...
## https://github.com/sofiabroome/painface-recognition/blob/master/i3d.py
...
def Inception_Inflated3d(include_top=True,
    ...
    # Arguments
        ...
        feat_extractor: optional to return the model at the AveragePooling3D layer
            to work include_top must be True
    # Returns
    ...
    if weights in WEIGHTS_NAME:
        ...
        if include_top and feat_extractor:
            xfeat_layer = model.get_layer('global_avg_pool')
            feat_model = Model(inputs=model.input, outputs=xfeat_layer.output, name='i3d_inception_{}'.format(data_str))
            return feat_model
