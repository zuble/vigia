...
def get_weight_from_ckpt(h5_folder_nd_file,config):
    ...
    model = form_modelrbg(config)

    full_ckpt_path = os.path.join(globo.CKPT_PATH,h5_folder_nd_file)
    ...
    model_weights_path = globo.WEIGHTS_PATH + model_name + '_weights.h5'
...
def init_modelrgb(params,from_path,pb=False,run=None):
    '''
    ...
    if from_path == str(globo.RGB_PATH):
        model = form_modelrbg(params)
    
        find_string=[params["ativa"]+'_'+params["optima"]+'_'+str(params["batch_type"])+'_'+params["frame_max"]]
        para_file_name, para_file_path = find_h5(from_path,find_string)
