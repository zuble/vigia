...
@tf.function
def vas_predict(x): return model(x, training=False)    
def predict_batch(frame_queque,vas_queque,ref_frame_array,total_frames,printt=False):
    ...
    #TO DO 
    # A SIMPLER WAY OF HANDLING END BATCHES
    # check if buffer frame max is > than total_frames
    #if frame_buffer_capacity >= total_frames:
    #    predict_at_close = True
...
def ASPlayer(video_path,toassert=False,toprint=False):
    ...
    prediction_thread = threading.Thread(target=predict_batch, args=(frame_queque , vas_queque , ref_frame_array, total_frames, toprint))
    ...
    while video.isOpened() :
        ...
        if sucess:
            ...
            # Get as if ready
            try: vas_atual = vas_queque.get_nowait() # Use this to prefer smooth display over frame/text shift
            except: pass
            
            
            # window prints
            cv2.putText(frame,'AS '+str('%.4f' % (vas_atual)),(10,15),font,fontScale+0.2,[0,0,255],thickness,lineType)
