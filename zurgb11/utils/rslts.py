...
def reorder_scv(desired_header , folder_path = globo.HIST_PATH):
    
    def process_csv(input_file, output_file, desired_header):
        with open(input_file, newline='') as csvfile:
            reader = csv.reader(csvfile)
            current_header = next(reader)

            # Check if the current header matches the desired header (ignoring extra columns)
            if desired_header == current_header: return False
            
            # Create a mapping to the desired header, ignoring extra columns
            column_mapping = [current_header.index(field) if field in current_header else None for field in desired_header]
            
            with open(output_file, 'w', newline='') as outfile:
                writer = csv.writer(outfile)
                writer.writerow(desired_header)
                
                # Write the remaining rows with the reordered columns, ignoring extra columns
                for row in reader:
                    reordered_row = [row[i] if i is not None else '' for i in column_mapping]
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.endswith('.csv'):
            ...
            output_file = os.path.join(folder_path, f'reordered_{filename}')
            
            # Process the input CSV file and write the reordered columns to the output file
            changed = process_csv(input_file, output_file, desired_header)

            # Replace the original file with the reordered one if changes were made
            if changed:
                os.replace(output_file, input_file)
                print(f'Processed and updated: {input_file}')
            else:
                print(f'Skipped (header already matches): {input_file}')
