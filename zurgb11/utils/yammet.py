...
class Yammet:
    def __init__(self, model_config):
        ...
        ## MODEL & METADATA
        self.model = es.TensorflowPredictVGGish(
    ...
    def get_sigmoid(self, vpath , sf = 0 , ef = -1 , debug = False):
        ...
        if debug: 
            tt2 = time.time()
            print(  "\n************** START yammet.get_sigmoid ***************",\
                    "\n\t",os.path.basename(vpath)," w/ ", mp4_fs_aac ,"hZ",\
                    "\n\t",sf , format(st, ".2f") ,\
                    "\n\t",ef , format(et, ".2f") )
                    #"\n\t MAX aas for anom labels 2")
            
            #for i in range(len(self.model_config['anom_labels_i'])):
            #    label_i = self.model_config['anom_labels_i'][i]
            #    print("\t",label_i, self.model_config['anom_labels'][i], np.amax(np.asarray(p_es)[:, label_i]))
            print('\n\n\t p_es @ yammet', np.shape(p_es),'@',str(tt2 - tt1),"secs") #"expls", np.amax(p_es[:, 72])
            print("\n***************** END yammet.get_sigmoid ********************")
    ...
    def get_sigmoid_fl(self, vpath , frame_intervals , debug = False):
        ...
        if debug: 
            ...
            for j in range(len(p_es_array)):
                ...
                #    print("\t",label_i, self.model_config['anom_labels'][i],"@max",np.amax(p_es_array[j][:, label_i]),"AVG",np.mean(p_es_array[j][:, label_i], axis=0))
                    
            print('\n\n\t p_es_array @ yammet', np.shape(p_es_array),'@',str(tt2 - tt1),"secs") #"expls", np.amax(p_es[:, 72])
            print("\n***************** END yammet.get_sigmoid ********************")
        else:
             for j in range(len(p_es_array)):
                print("interval",j,"@",np.shape(p_es_array[j]),frame_intervals[j][2])   
