...
def name_it(cfg,string_it=False):
    aa = cfg["backbone"]+'_'+cfg["ativa"]+'_'+cfg["optima"]+'-'+str(cfg["lr"])+'_'+str(cfg["batch_type"])+'_'+str(cfg["frame_step"])+'_'+str(cfg["frame_max"])
...
def form_model(params):
    ...
    image_input = keras.Input(shape=(None, *params["in_shape"] ))
    print(params["backbone"],params["in_shape"])
    
    if params["backbone"] == 'original':
        #in_height = 120; in_width = 160
        #input_shape = (in_height, in_width, 3)
        #image_input = keras.Input(shape=(None, *input_shape), name='input_layer')
        # ( 1 , frame_max , h , w , ch)    
        # ( 1 , 1000 , 120 , 160 , 3)
        ...
        backbone_out = c3d_mp(image_input)
        ...
        backbone = tf.keras.applications.ResNet50(include_top=False, weights='imagenet', input_shape = params["in_shape"])
        backbone_out = keras.layers.TimeDistributed(backbone)(image_input)
          
    elif params["backbone"] == 'mobilenetv2':
        #input_shape = (224, 224, 3)
        #image_input = keras.Input(shape=(None, *input_shape))
        backbone = tf.keras.applications.MobileNetV2(include_top=False, weights='imagenet', input_shape = params["in_shape"])
        backbone_out = keras.layers.TimeDistributed(backbone)(image_input)
        
    elif params["backbone"] == 'xception':
        #input_shape = (299, 299, 3)
        #image_input = keras.Input(shape=(None, *input_shape))
        backbone = tf.keras.applications.Xception(include_top=False, weights='imagenet', input_shape = params["in_shape"])
        backbone_out = keras.layers.TimeDistributed(backbone)(image_input)
        
    else: raise ValueError("'original' 'resnet50' 'mobilenetv2' or 'xception'.")
    
    
    if params["backbone"] != 'original':
        print("\n\tsetting non trainable") 
        for layer in backbone.layers: 
            layer.trainable = False
    
    
    features_flatten = keras.layers.Lambda(all_operations)(backbone_out)  # flatten spatial features to time series
    ...
    hidden_dense_1 = keras.layers.Dense(256, activation=ativa)(global_rgb_feature)
    hidden_dense_2 = keras.layers.Dense(128, activation=ativa)(hidden_dense_1)
    sigmoid = keras.layers.Dense(1, activation='sigmoid', name='output_layer')(hidden_dense_2)
    ...
    elif params["optima"]=='adagrad':optima=tf.keras.optimizers.Adagrad(learning_rate=params["lr"])
