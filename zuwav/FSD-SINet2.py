...
def FSDSINET_atuator(mp4path, vaf_queque , asmp_queque , ases_queque ,test_conf):
    ...
    while True:
        ...
        if vaf == test_conf["batch_len"] + test_conf["batch_step_len"] * batch_step_atual:
            ...
            asmp_queque.put((top_mp,batch_step_atual))
            ...
            ases_queque.put((top_es,batch_step_atual))    
...
def FSDSINET_player(index, mp4path):
    ...
    FSDSINet_atuactor_thread = threading.Thread(target=FSDSINET_atuator, args=(mp4path , vaf_queque , asmp_queque , ases_queque , test_config,) )
    ...
    while True:
        ...
        if not ret:break
            ...
            # Get aas if ready
            try: top_mp3,batch_step_atual = asmp_queque.get_nowait()
            except: pass
            try: top_es3,batch_step_atual = ases_queque.get_nowait()
            except: pass    
