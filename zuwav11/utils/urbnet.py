...
class Urbnet:
    ...
    def get_sigmoid(self, vpath , sf = 0 , ef = -1 , debug = False):
        ...
        if debug: 
            ...
            print('\n\n\t p_es @ urbnet', np.shape(p_es),'@',str(tt2 - tt1),"secs") #"expls", np.amax(p_es[:, 72])
            print("\n***************** END urbnet.get_sigmoid ********************")
    ...
    def get_sigmoid_fl(self, vpath , frame_intervals , debug = False):
        ...
        if debug: 
            ...
            print('\n\n\t p_es_array @ urbnet', np.shape(p_es_array),'@',str(tt2 - tt1),"secs") #"expls", np.amax(p_es[:, 72])
            print("\n***************** END urbnet.get_sigmoid ********************")
        else:
             for j in range(len(p_es_array)):
                print("interval",j,"@",np.shape(p_es_array[j]),frame_intervals[j][2])   
